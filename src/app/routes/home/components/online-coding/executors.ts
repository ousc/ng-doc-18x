...
export const deleteTask2 = (data: any[], callBack: (data: any[]) => void) => {
  const affectRows = data.filter(item => item.category === "Fashion" && item.quantity < 3).length;
  callBack(data.filter(item => item.category === "Fashion" && item.quantity < 3));
  return [
    `---------------Kronos Task---------------`,
...
export const deleteTask3 = (data: any[]) => [
  `---------------Kronos Task---------------`,
  `Sql: \t${'DELETE FROM `tb_product` WHERE `code` = :code'}`,
  `Params: {code: "a3c2a3fv27"}`,
  `-----------------------------------------`,
  `Result: affected rows: ${data.filter(item => item.code === "a3c2a3fv27").length}`
]
